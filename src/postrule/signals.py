...
class SignalSignature:
    ...
    def core_identity(self) -> tuple:
        ...
        retrain (which bumps the version) is not misread as a signal swap."""
        return (self.judge_id, self.gate_id, self.strategy_id)

    def changed_components(self, other: SignalSignature) -> list[str]:
        """Which of judge / gate / strategy differ from ``other``."""
        changed = []
    ...
    def digest(self) -> str:
        ...
        canonical = json.dumps(
            [self.judge_id, list(self.gate_id), _jsonable(self.strategy_id)],
            sort_keys=True,
            default=str,
        )
