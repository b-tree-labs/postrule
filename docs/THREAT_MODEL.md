# Postrule threat model

This document describes Postrule's trust boundaries and the threats considered
...
adopter who wants to know **what Postrule defends against by design**, **what
...
Last reviewed: 2026-05-04 against `src/postrule/` at HEAD.

## What ships today

Postrule v1.0 is **a Python library and CLI** plus a small hosted account
surface for account creation, API keys, and billing:

- **Client SDK** (Apache 2.0) — `import postrule` into your own process.
...
- **In-process analyzer + ROI + research tooling** (BSL 1.1) — the
  `postrule` CLI: `postrule analyze`, `postrule init`, `postrule roi`,
  `postrule bench`, `postrule plot`, `postrule quickstart`. **Runs locally
  in the user's process; not a network service.**
- **Hosted account surface** — a Cloudflare-hosted dashboard
  (`app.postrule.ai`) and Worker (`api.postrule.ai`) for account creation
  (Clerk auth), API key issuance, and billing (Stripe). These exchange
  metadata only; **no source code, prompt content, or labels are
  transmitted** by the v1.0 client to any Postrule-operated service.

There is **no Postrule-hosted analyzer service** in v1.0 — the Analyzer
...
**Status today:** the Analyzer (`src/postrule/analyzer.py`) is a
local-only static AST scanner that runs in the user's own Python
process via `postrule analyze`. There is **no network protocol, no
...
custom serialization. Postrule ships **none of this enabled by default**;
...
| `postrule.bundled` (llama-cpp-python) | local — GGUF is lazy-downloaded once from the Postrule CDN; subsequent calls are local-only |
...
The v1.0 hosted account surface (`api.postrule.ai` Worker +
`app.postrule.ai` dashboard) handles account creation, API key
...
to any Postrule-operated service.**
...
**Hosted Analyzer (`[v1.1]`):** if a Postrule-hosted analyzer is
...
|---|---|---|
| Compromised verifier model returns adversarial verdicts | Medium | Distinct-from guardrail: `JudgeSource(require_distinct_from=...)` rejects construction at switch-build-time when classifier and judge resolve to the same `(adapter_class, model_string)` pair (`src/postrule/verdicts.py::_same_model`). Multi-judge consensus via `JudgeCommittee` (majority / unanimous / confidence-weighted modes) bounds single-judge influence. |
...
| Token / API-key exfiltration | Medium | Standard secret hygiene only — Postrule reads `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` from the environment and never persists them. Operator-side: rotate keys on a defined cadence, store in a secret manager rather than `.env` files in git, and prefer per-environment short-lived keys where the provider supports them. |
...
- **Side-channel attacks on the host running the SDK.** Timing
  ...
  attacks against the model, etc. — Postrule inherits the host's
...
- **Physical security of self-hosted deployments.** Postrule trusts
...
- **DOS / abuse rate-limiting at the SDK layer.** Postrule is a
...
audit trails, regulated-decision provenance, ROI claims). Postrule's
...
- The audit chain is **not cryptographically signed**. A privileged
  ...
  tamper-evidence, layer Postrule on top of an append-only store with
  cryptographic guarantees (e.g., AWS QLDB, an immutable object-
  store bucket with object-lock, a signed-log service) via a custom
  `Storage` implementation. Postrule's `Storage` protocol is
  intentionally minimal so this composition is straightforward.

The full `Storage` protocol contract is in `src/postrule/storage.py`
...
**`security@postrule.ai`** — do not open a public GitHub issue for
