...
**Drift detection.** Lu et al. (2018) and Gama et al. (2014) survey concept-drift adaptation. Drift detection is *complementary* to graduation: graduation answers when to first migrate; drift detection answers when the migrated classifier has degraded enough that fallback to the rule is justified. The same paired-test machinery that justifies advancement justifies demotion when its arguments are reversed (current decision-maker compared against the rule), so the v1 reference implementation ships an autonomous demotion path on the accuracy axis driven by the same Gate primitive defined in §3. The empirical demotion-curve analog of the transition curves we report (how much drift accumulates before the gate fires) and full drift-driven re-graduation across the lifecycle are follow-on research; here we establish that the primitive is direction-agnostic and the safety theorem (§3.3, Remark) applies symmetrically.
...
*In plain English: if the candidate is actually no better than the incumbent, the gate falsely promotes it at most $\alpha$ of the time. At $\alpha = 0.01$, that is a 1% worst-case promotion-error rate per evaluation, by design.*
...
*In plain English: when the candidate is not truly better than the incumbent, the rows where the two classifiers disagree split roughly 50/50 between "candidate right" and "incumbent right." The McNemar test only fires on a strong tilt toward the candidate, and the math of the binomial null bounds the chance of seeing such a tilt by accident at $\alpha$.*
...
*In plain English: three statistical checkpoints separate the rule from full ML autonomy; each has at most a 1% worst-case false-promotion rate at $\alpha = 0.01$. The joint worst case across all three is 3%; the single canonical step (handing the wheel to ML) stays at 1%. That sits in the same range as the false-failure rate teams already tolerate from production CI tests.*
