...
Production classification systems overwhelmingly start as hand-written rules — support-ticket triage, intent routing, content moderation, retrieval-strategy selection — because training data does not yet exist on day one. Over time, outcome data accumulates, yet the rules stay frozen: replacing a rule with a machine-learned classifier requires custom migration engineering at every decision point, and there is no shared statistical criterion for when the migration is justified. We formalize this migration as a *graduated-autonomy lifecycle* with six phases (`RULE` → `MODEL_SHADOW` → `MODEL_PRIMARY` → `ML_SHADOW` → `ML_WITH_FALLBACK` → `ML_PRIMARY`) in which the rule remains a structural safety floor at every stage. We prove that, when transitions are gated by a paired McNemar test at significance level $\alpha$, the marginal probability of a worse-than-rule transition is bounded above by $\alpha$ per gated step. We empirically characterize the *transition curve* — accumulated outcome volume against the depth at which a graduated classifier first beats the rule floor at $p < 0.01$ — across four public intent-classification benchmarks (ATIS, HWU64, Banking77, CLINC150). Two regimes emerge cleanly: a low-cardinality narrow-domain regime where rules give a usable baseline that ML eventually beats by ~19 points, and a high-cardinality broad-domain regime where rules are non-viable from day one (0.5–1.8% accuracy) and the entire ML migration is gated by outcome-data accumulation rather than statistical significance. Every benchmark crosses paired McNemar significance at the *first* checkpoint of 250 outcomes. We release a reference implementation (the Dendra library), the full transition-curve dataset, and the benchmark harness that reproduces the result.
...
**Drift detection.** Lu et al. (2018) and Gama et al. (2014) survey concept-drift adaptation. Drift detection is *complementary* to graduation: graduation answers when to first migrate; drift detection answers when to renew the migration. Both are downstream consumers of the same outcome log. Drift-driven re-graduation is a natural extension of this work that we leave to follow-on research.
...
We do not claim to solve drift. The lifecycle migrates rule → ML once. Distribution shift over time will eventually erode the ML head's accuracy below the rule's, at which point a Bayesian update of the gate or a fresh evaluation is required. Drift detection (Lu et al., 2018; Gama et al., 2014) is the complementary literature; integrating it as a `DriftGate` or as a counter-direction transition $P_5 \to P_4 \to P_3$ is a natural extension.
...
- **Drift integration.** A `DriftGate` would fire on distribution shift to demote a phase or escalate to re-evaluation. The outcome log is the substrate; the integration is mechanical.
...
- **Theoretical strengthening.** The safety theorem is per-transition. A stronger statement — that the joint distribution over transitions admits a better-than-union-bound guarantee — would require characterizing the dependence between gates. Tzamos et al.'s theory work on classifier-comparison testing is the relevant adjacent literature.
