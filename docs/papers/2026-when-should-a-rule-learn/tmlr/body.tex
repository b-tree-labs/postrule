...
\subsection{1.1 The pattern nobody has
formalized}\label{the-pattern-nobody-has-formalized}

Open any production codebase. There are functions that take an input and
return one of a fixed set of labels:
\texttt{triage\_ticket(t)\ →\ \{bug,\ feature\_request,\ question\}};
\texttt{route\_intent(u)\ →\ one\ of\ 77\ banking\ labels};
\texttt{classify\_content(c)\ →\ \{safe,\ review,\ block\}};
\texttt{pick\_retrieval\_strategy(q)\ →\ \{dense,\ sparse,\ hybrid\}};
\texttt{detect\_language(snippet)\ →\ \{python,\ javascript,\ fortran,\ ...\}}.

On day zero these functions are written as hand-rules
(\texttt{if\ "crash"\ in\ title:\ return\ "bug"}) because no training
data exists yet, the engineer has 100 examples to look at, and the
deadline is now. The decision quality is whatever the rule's keyword
coverage admits.

By day \(N\) outcome data has accumulated. CSAT surveys, resolution
codes, downstream audit trails, user corrections, A/B test conversion
data: production systems generate a stream of evidence that lets the
engineer compute, for each historical classification, whether the rule
was right. A learned classifier trained on that evidence would, in
principle, do better.
...
\item
  \textbf{Rules calcify.} Distribution shifts that would be obvious to a
...
\item
  \textbf{ML-from-day-one fails.} Without sufficient training data,
...
\item
  \textbf{Ad-hoc migration has no contract.} Most production
  \texttt{try\_ml\_else\_fall\_back\_to\_rule} code embeds an implicit
  confidence threshold and an implicit assumption that ML is at least as
  good as the rule. Both go untested. Breck et al.~(2017) provide a
  rubric (the ML Test Score) for production-readiness;
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{A six-phase graduated-autonomy lifecycle} (Table 1) with
...
\item
  \textbf{A safety theorem} (Theorem 1, §3) bounding the per-transition
...
\item
  \textbf{Empirical transition curves on eight public benchmarks across
...
\item
  \textbf{An open-source reference implementation} (the \sysname{} Python
...
target reliably better than the current decision-maker?'', not ``should
we advance?''), so it generalizes both to the demotion direction (drift
...
\textbf{LLM cascade routing.} The closest contemporary lineage is the
...
routing from preference data, recovering 95\% of GPT-4 quality at 15\%
of the cost on MT-Bench. \emph{A Unified Approach to Routing and
Cascading for LLMs} (Dekoninck et al., 2025) provided a theoretical
unification, framing both as instances of a meta-classifier over a model
pool with formal optimality conditions.
...
\textbf{Statistical methodology, paired tests for ML.} The McNemar test
...
\textbf{Production ML safety.} Sculley et al.~(2015) document the
...
\textbf{LLM-as-judge and evaluation harnesses.} The verdict-source layer
...
\texttt{JudgeSource} self-judgment guardrail. Chiang et al.~(2024)
provide the \emph{Chatbot Arena} preference data that is the substrate
for downstream learned routers like RouteLLM.

Recent practitioner work has emphasized the \emph{harness} as a
first-class object: Trivedy (2026) describes ``harness hill-climbing''
as the iterative discipline of evaluating evaluation-harnesses
themselves, and Martin's \emph{Auto-Evaluator} tooling at LangChain
(2023) was an early production realization of LLM-as-judge for
retrieval-quality measurement. Our \texttt{CandidateHarness} (§9.5) is
in this lineage: a substrate for autoresearch loops that propose,
evaluate, and either promote or discard candidate classifiers under a
statistical gate.

\textbf{AutoML and online learning, for differentiation.} AutoML (Hutter
...
\textbf{Cascade learning, historical lineage.} The cascade pattern
...
\textbf{Agent / autoresearch loops.} Recent work has popularized the
...
Our \texttt{CandidateHarness} is the production substrate underneath: it
...
\textbf{Calibration.} Guo et al.~(2017) document that modern neural
networks are systematically miscalibrated, motivating the explicit
\texttt{confidence\_threshold} parameter in our adapter layer. Kuleshov
et al.~(2018) extend calibration analysis to deep regression.
Calibration is a load-bearing assumption when the cascade pattern routes
by confidence; we do not contribute to this literature, but we cite it
because the system depends on it being well-handled at the adapter
boundary.

\textbf{Drift detection.} Lu et al.~(2018) and Gama et al.~(2014) survey
...
\textbf{Head-to-head comparison.} Against the four named alternatives a
practitioner is most likely to weigh against \sysname{}:
...
Property (on a Banking77-shape site)
...
Statistical guarantee on transitions? & yes (paired McNemar,
\(\alpha\)-bounded) & no & no (learned router) & n/a & no \\
Eventually graduates off LLM? & yes (\(P_4 \to P_5\)) & no (cascade is
permanent) & no (router is permanent) & n/a (no LLM) & no \\
Rule safety floor? & yes (structural, \(P_0\) through \(P_4\)) & no & no
& no & partial (manual) \\
Sub-millisecond final state? & yes (in-process sklearn at \(P_5\)) & no
(LLM in critical path) & no (LLM in critical path) & yes & depends \\
Per-site lifetime decision? & yes (one decorator per site) & per-call
routing & per-call routing & per-site training run & per-site tuning \\
\end{longtable}
}

\textbf{Table.} \emph{Head-to-head feature comparison.} Cascade routing
(FrugalGPT) and learned routing (RouteLLM) optimize inference-time
selection among an already-trained model pool; both keep the LLM tier
permanently in the critical path. Raw ML-from-day-one bypasses the
cold-start problem only when day-one labels exist. Ad-hoc thresholding
is what most production code defaults to without a primitive. \sysname{}
is the only entry that combines a day-zero rule floor, a Type-I-bounded
statistical gate, and an explicit graduation off the LLM tier.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Formal Framework}\label{formal-framework}

\subsection{3.1 The graduated-autonomy
lifecycle}\label{the-graduated-autonomy-lifecycle}
...
Formally, we define a \emph{learned switch} over a label set
\(\mathcal{L}\) as a tuple \(S = (R, M, H, \phi)\) where
\(R: \mathcal{X} \to \mathcal{L}\) is the rule,
\(M: \mathcal{X} \to (\mathcal{L}, [0,1])\) is an optional model
classifier returning a label and confidence,
\(H: \mathcal{X} \to (\mathcal{L}, [0,1])\) is an optional ML head, and
\(\phi \in \{P_0, \ldots, P_5\}\) is the lifecycle phase. The decision
function is:
...
\bottomrule\noalign{}
\endlastfoot
\(P_0\) (\texttt{RULE}) & \(R(x)\) & self \\
\(P_1\) (\texttt{MODEL\_SHADOW}) & \(R(x)\); \(M\) logged but inert &
floor \\
\(P_2\) (\texttt{MODEL\_PRIMARY}) & \(M(x)\) if
\(\text{conf}_M \ge \theta\); else \(R(x)\) & fallback \\
\(P_3\) (\texttt{ML\_SHADOW}) & \(M(x)\) if \(\text{conf}_M \ge \theta\)
else \(R(x)\); \(H\) logged but inert & floor \\
\(P_4\) (\texttt{ML\_WITH\_FALLBACK}) & \(H(x)\) if
\(\text{conf}_H \ge \theta\); else \(P_3\)'s decision & fallback \\
\(P_5\) (\texttt{ML\_PRIMARY}) & \(H(x)\) & circuit-breaker only \\
\end{longtable}
}

\textbf{Table 1.} \emph{The six-phase graduated-autonomy lifecycle.}
Three transitions (\(P_2 \leftarrow P_1\), \(P_4 \leftarrow P_3\),
\(P_5 \leftarrow P_4\)) are statistically gated; two
(\(P_1 \leftarrow P_0\), \(P_3 \leftarrow P_2\)) are operator- or
construction-driven (shadow modes are no-risk additions of an inert
candidate). The rule \(R\) is structurally preserved in every phase
except \(P_5\), where it remains as the circuit-breaker target on
detected ML failure. A \texttt{safety\_critical=True} configuration caps
the lifecycle at \(P_4\), refusing \(P_5\) at construction time.

\textbf{Predecessor-cascade fallback.} The decision rule for \(P_4\) is
recursive: low-confidence \(H\) falls through to \(P_3\)'s decision,
which itself falls through to \(P_2\)'s decision, which falls to \(R\).
Expanded, \(P_4\)'s routing is \(H(x)\) if \(\text{conf}_H \ge \theta\),
else \(M(x)\) if \(\text{conf}_M \ge \theta\), else \(R(x)\). The
...
\(M\) to save token cost after graduating to \(P_4\)), the cascade
collapses gracefully to \(H\)-then-\(R\), identical to the simpler
two-tier behavior. The structural symmetry across phases (each
transition adds exactly one tier above its predecessor) is what makes
the safety theorem (§3.3) extend cleanly to multi-tier cascades: the
rule fires when every learnable tier above it is below threshold.

\begin{figure}
\centering
\pandocbounded{\includegraphics[width=\linewidth]{results/figure-5-lifecycle.png}}
\caption{Figure 5. The graduated-autonomy lifecycle: solid arrows are
gate-driven advances; dashed arrows are demotions when the same gate
machinery fires in the reverse direction on accumulated drift. The rule
R is the structural floor in P1--P4 and the circuit-breaker target in
P5.}
\end{figure}

\subsection{3.2 Transition guards}\label{transition-guards}

Each gated transition \(P_k \to P_{k+1}\) admits a guard
\(G_k(\mathcal{D})\) that returns \texttt{advance\ \textbar{}\ hold}
over an outcome dataset \(\mathcal{D}\). The default guard is the paired
McNemar test, which counts only the rows where the two classifiers
disagree and asks whether the disagreements are lopsided enough toward
the candidate to be unlikely by chance.
...
operator picks (we default to 1\%); the minimum-pair threshold
\(n_{\min}\) keeps the gate from firing on too few disagreements to be
reliable; and the directional condition rules out ties, so the gate
stays put if the two classifiers are equally good. Together these three
turn ``should we replace this rule'' from an intuitive call into a
reproducible, auditable decision.

\textbf{Definition (paired McNemar gate).} Given two classifiers \(A\)
...
The paired McNemar test rejects ``\(A\) and \(B\) have equal accuracy''
at significance level \(\alpha\) when the two-sided exact binomial
\(p\)-value of \(\min(b, c)\) under the null
\(\text{Binomial}(b + c, 0.5)\) is below \(\alpha\). The gate
\$G(\mathcal{D}) = \$ \texttt{advance} iff \(b > c\) and \(p < \alpha\)
and \(b + c \ge n_{\min}\).

The minimum-pair condition \(n_{\min}\) (we use \(n_{\min} = 200\))
prevents a runaway low-volume rejection. The directional condition
\(b > c\) ensures the test only advances when the candidate is \emph{the
better} classifier, not merely when the two differ.
...
\subsection{3.3 Safety theorem}\label{safety-theorem}
...
replace, the gate has at most a 1\% chance of wrongly promoting it on
any single evaluation at \(\alpha = 0.01\), by construction of the
paired McNemar test's null distribution.
...
\textbf{Theorem 1 (per-transition safety).} Let \(G\) be a paired
McNemar gate at significance \(\alpha\) with minimum-pair threshold
\(n_{\min}\). Let \(A\) be the incumbent classifier and \(B\) the
candidate, both evaluated on a stream of paired-correctness rows from
the same input distribution \(\mathcal{X}\). If \(B\) has true accuracy
on \(\mathcal{X}\) no greater than \(A\), then the probability that
\(G\) advances is bounded above by \(\alpha\).

\emph{Proof sketch.} The paired McNemar test's null hypothesis is
``\(P(B \text{ right} | A \text{ wrong}) = P(A \text{ right} | B \text{ wrong})\),''
equivalently ``\(E[b] = E[c]\)'' over the discordant-pair distribution.
If \(B\) is no better than \(A\) in true accuracy on \(\mathcal{X}\),
the distribution of discordant pairs satisfies \(E[b] \le E[c]\), so
\(\min(b, c) = b\) in expectation. The rejection rule rejects when the
two-sided binomial \(p\)-value of \(\min(b, c)\) falls below \(\alpha\);
under the null and the directional condition \(b > c\), this happens
with probability at most \(\alpha / 2\). The marginal advance
probability is therefore at most \(\alpha\). \(\blacksquare\)

\textbf{Corollary (lifecycle safety).} With three statistically-gated
...
by \(3\alpha\) (union bound), and exactly \(\alpha\) for the single
canonical transition the field most cares about (\(P_4 \to P_5\)). At
\(\alpha = 0.01\), this is a 1\% per-step worst-case ceiling, the same
order of magnitude as the conservative regression-test FPR teams already
accept in production CI/CD.

\textbf{Remark (direction-agnosticism and n-axis generalization).} The
theorem statement is symmetric in \(A\) and \(B\): the gate's question
is ``is the comparison target reliably better than the current
...
\subsection{3.4 Why this gate, not an
alternative}\label{why-this-gate-not-an-alternative}
...
\tightlist
\item
  \textbf{Unpaired two-proportion z-test.} The version most often seen
...
\item
  \textbf{Accuracy margin} (``ML beats rule by \(\ge \delta\)'').
  Simple, but has no Type-I-error interpretation; reviewers who care
  about formal claims will ask what \(\delta\) corresponds to.
\item
  \textbf{Bayesian decision theory} (e.g., learned routers from
  ...
  in a learned router via the \texttt{Gate} protocol when their data
  supports it.
\item
  \textbf{Composite gates} (logical conjunctions, e.g., paired-McNemar
  AND minimum-volume AND accuracy-margin). Available as
  \texttt{CompositeGate} in the reference implementation. Useful when
  domain rules require an additional floor; we report the paired-McNemar
  single-gate result as the default.
\end{enumerate}
...
\subsection{4.1 Benchmarks}\label{benchmarks}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{ATIS} (Hemphill et al., 1990) & 26 & 4,978 & 893 & Single
(flight booking) \\
\textbf{HWU64} (Liu et al., 2019) & 64 & 8,954 & 1,076 & Multi (21
scenarios) \\
\textbf{Banking77} (Casanueva et al., 2020) & 77 & 10,003 & 3,080 &
Single (banking) \\
\textbf{CLINC150} (Larson et al., 2019) & 151 & 15,250 & 5,500 & Multi
(10 + OOS) \\
\end{longtable}
}

\textbf{Table 2.} \emph{Headline benchmarks.} All are public,
...
\subsection{4.2 Rule construction}\label{rule-construction}

The day-zero rule is automated and deliberately simple. For each
dataset, \texttt{postrule.benchmarks.rules.build\_reference\_rule}
constructs an \texttt{if/elif} cascade by:
...
\item
  ...
  frequency (TF-IDF).
\item
  Generating an \texttt{if\ keyword\ in\ input:\ return\ label} cascade
  in lexical order.
\item
  Falling back to the modal label in the seed window (or
  \texttt{out\_of\_scope} for CLINC150).
\end{enumerate}
...
\subsection{4.3 ML head}\label{ml-head}

The ML head for the §5.1 headline experiment is
\texttt{postrule.ml.SklearnTextHead}: TF-IDF features with sublinear
...
heads (\texttt{TfidfLinearSVCHead}, \texttt{TfidfMultinomialNBHead},
\texttt{TfidfGradientBoostingHead}) and one non-text head
(\texttt{ImagePixelLogRegHead}, used for the §5.7 CIFAR-10 bench). §5.6
reports the autoresearch result that the empirical winner is
...
\subsection{4.4 Verdict simulation}\label{verdict-simulation}
...
\item
  Apply the paired McNemar gate (\(\alpha = 0.01\), \(n_{\min} = 200\))
  to the test-set paired-correctness arrays.
\end{enumerate}

This produces the \emph{transition curve} (outcome volume against
accuracy and against gate-statistic) for each benchmark.

\subsection{4.5 Metrics}\label{metrics}

\begin{itemize}
\tightlist
\item
  \textbf{Final ML accuracy.} Test-set accuracy after the full training
  stream.
\item
  \textbf{Transition depth.} The smallest checkpoint at which the paired
  McNemar gate advances at \(p < 0.01\). This is the headline metric.
\item
  \textbf{Crossover delta.} ML accuracy minus rule accuracy at the
  transition point.
\item
  \textbf{McNemar discordant pair counts} (\(b\), \(c\)) at the final
  checkpoint, the underlying statistical evidence.
\end{itemize}

\subsection{4.6 Reproducibility}\label{reproducibility}

All code is at \texttt{{[}anonymized\ repository{]}} (Apache 2.0). All
fixed seeds are documented in the benchmark JSONL output. The
transition-curve dataset is released as
\texttt{postrule-transition-curves-2026.jsonl} accompanying this paper.
The benchmark harness ships in
\texttt{src/postrule/research.py::run\_benchmark\_experiment} and the
McNemar computation in \texttt{src/postrule/gates.py::McNemarGate}.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Transition Curves: Main
Results}\label{transition-curves-main-results}

\subsection{5.1 Headline: transition curves across the eight-benchmark
suite}\label{headline-transition-curves-across-the-eight-benchmark-suite}
...
First clear (\(p < 0.01\))
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
Total outcomes to ML final
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\textbf{codelangs} (\sysname{} 2026) & code & 12 & 87.8\% & 97.1\% &
\emph{(extract)} & \textbf{97.8\%} & \textbf{400} & 553 \\
\textbf{ATIS} (Hemphill et al.~1990) & flight booking & 26 & 70.0\% &
75.6\% & 81.9\% & \textbf{88.7\%} & \textbf{250} & 4,978 \\
\textbf{TREC-6} (Li \& Roth 2002) & question type & 6 & 43.0\% & 60.6\%
& 70.8\% & \textbf{85.2\%} & \textbf{250} & 5,452 \\
\textbf{AG News} (Zhang et al.~2015) & news topics & 4 & 25.9\% & 46.9\%
& 46.9\% & \textbf{91.8\%} & \textbf{1,000} & 120,000 \\
\textbf{Snips} (Coucke et al.~2018) & voice assistant & 7 & 14.3\% &
27.4\% & 0.0\% & \textbf{98.2\%} & \textbf{2,000} & 13,084 \\
\textbf{HWU64} (Liu et al.~2019) & multi-domain assistant & 64 & 1.8\% &
2.7\% & 10.5\% & \textbf{83.6\%} & \textbf{250} & 8,954 \\
\textbf{Banking77} (Casanueva et al.~2020) & banking & 77 & 1.3\% &
2.6\% & 8.8\% & \textbf{87.7\%} & \textbf{250} & 10,003 \\
\textbf{CLINC150} (Larson et al.~2019) & multi-domain + OOS & 151 &
0.5\% & 1.6\% & 5.2\% & \textbf{81.9\%} & \textbf{250} & 15,250 \\
\end{longtable}
}

\textbf{Table 3.} \emph{Headline transition-curve results across the
eight-benchmark suite.} Sorted by descending rule baseline. Every
benchmark crosses paired-McNemar significance at \(\alpha = 0.01\). The
``ML @ first clear'' column reports the ML head's test accuracy at the
gate-fire moment (separately from the asymptote at training-set
exhaustion); ``ML final (full train)'' is the asymptote after the entire
training corpus is consumed. Banking77, HWU64, CLINC150, and Snips at
seed=100 with the as-shipped HuggingFace split produce single-label
(modal-fallback) rules. Their \texttt{Rule\ acc} column equals chance
(1/k). Under random shuffling of the training stream the rule recovers
to 17 to 75 percent depending on benchmark (Table 4b). The
chance-accuracy floor on these four rows is therefore a property of
stream ordering at seed=100, not of the benchmark itself. Snips is the
one outlier on first-clear depth: the as-shipped rule's 14.3\% accuracy
beats the ML head at the 250-outcome checkpoint, and the advance gate
fires only at 2,000 outcomes after ML overtakes.

Two views of the data are useful and pull complementary signals.

\begin{figure}
\centering
\pandocbounded{\includegraphics[width=\linewidth]{results/figure-1-transition-curves.png}}
\caption{Figure 1. Transition curves across the eight-benchmark suite,
sorted by descending rule baseline. Top row (rule has fighting chance):
codelangs, ATIS, TREC-6, AG News. Bottom row (rule at the modal-fallback
floor under the as-shipped HuggingFace ordering): Snips, HWU64,
Banking77, CLINC150. Rule accuracy (grey, flat) and ML-head accuracy
(blue, rising) on a log x-axis; the dashed red vertical marks the first
paired-McNemar gate fire at p \textless{} 0.01. Shuffle-sensitivity for
the bottom-row rules is reported in Table 4b.}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[width=\linewidth]{results/figure-2-pvalue-trajectories.png}}
\caption{Figure 2. Paired McNemar p-value trajectories per benchmark,
log-scale y-axis. The dashed red horizontal at alpha = 0.01 is the gate
threshold. Every trajectory crosses it; the rate of descent reveals the
relative effect size per benchmark.}
\end{figure}

\subsection{5.2 Three regimes by cardinality and rule-keyword
affinity}\label{three-regimes-by-cardinality-and-rule-keyword-affinity}
...
\textbf{Regime I, \emph{rule near optimum} (codelangs).} With 12
programming-language labels and rigid syntactic keywords (\texttt{def},
\texttt{function}, \texttt{module}, \texttt{subroutine},
\texttt{package}), the auto-rule lands at 87.8\%, within ten points of
the ML ceiling at 97.8\%. At the first checkpoint (100 outcomes) rule
and ML differ by only 1.4 pp; the McNemar gate does not clear until 400
outcomes. The practitioner question shifts from \emph{``when should the
rule graduate?''} to \emph{``is graduation worth the engineering at
all?''} The lifecycle still helps (audit chain, structural fallback,
...
\textbf{Regime II, \emph{rule usable} (ATIS, TREC-6).} Mid-cardinality
(6 to 26 labels) with strong-to-moderate keyword affinity. ATIS at
70.0\% rule has 18× chance accuracy; TREC-6 at 43.0\% has 2.6× chance.
The rule is shippable. ML decisively wins (88.7\% on ATIS, 85.2\% on
TREC-6) and the McNemar gate clears at the very first checkpoint with
extreme p-values (\(p \approx 10^{-33}\) on ATIS at end-of-training,
\(\approx 10^{-16}\) at the first 250-outcome checkpoint on TREC-6).
This is the canonical ``rule today, gated graduation tomorrow'' case the
lifecycle was designed for.

\textbf{Regime III, \emph{rule starts at the modal-class floor} (HWU64,
...
should not infer ``this benchmark is hopeless for keyword rules'' from
the as-shipped baseline alone. Two distinct paths get a benchmark into
this regime:
...
\item
  ...
  stream the Snips rule recovers to 75\% (Table 4b); the as-shipped
...
wait on hand-labeled training data before launching. \sysname{}'s role
here is cold-start substrate, not graduation: outcome logging from day
one regardless of which decision-maker is in front, with an explicit
migration path to a trained ML head once enough data accumulates.

\begin{figure}
\centering
\pandocbounded{\includegraphics[width=\linewidth]{results/figure-3b-extended-regimes.png}}
\caption{Figure 3. Day-zero rule baseline (grey) vs final ML-head
accuracy (blue), all eight benchmarks, sorted by rule baseline.
Background tinting hints at absolute thresholds (green ≥ 50\%, olive
20--50\%, pink \textless{} 20\%); the §6 taxonomy refines this with
chance-relative classification. AG News at 25.9\% is barely above its
25\% chance baseline and belongs in Regime III alongside Snips and the
...
\tightlist
\item
  \textbf{Regime II user:} ``Our rule works; should we replace it?'' →
  ``Yes, here is the statistical evidence; the McNemar gate would
  advance you within the first 250 outcomes.''
\item
  \textbf{Regime III user (high cardinality or weak affinity):} ``We
  need an N-way classifier and we don't have training data yet.'' →
  ``Start at Phase 2 with a zero-shot LLM in front of \sysname{}'s
  outcome-logging layer; the log it generates is the training-data
  source for the eventual ML head.''
\item
  \textbf{Regime I user:} ``Our rule works very well; is graduation even
  worth the engineering?'' → ``The McNemar gate eventually clears, but
  the lifecycle's value here is in audit, structural fallback, and drift
  symmetry, not in raw accuracy lift. Decide based on per-decision
  miscost.''
\end{itemize}

\subsection{5.3 Seed-size sensitivity}\label{seed-size-sensitivity}

The Regime III (rule at floor) story rests on a 100-example seed feeding
\texttt{build\_reference\_rule}. A reviewer's natural question is
whether a more thorough engineer with 10× more examples to inspect would
build a rule that escapes the floor. We re-ran the four headline
benchmarks with a 10× seed (1,000 examples) to check:
...
\textbf{Table 4.} \emph{Rule sensitivity to seed size.} Even with 10×
...
Snips & 7 & 14.3\% & \textbf{75.3\%} & 68.8\% & 77.3\% & \textbf{+61.0
pp} \\
...
\textbf{Table 4b.} \emph{Rule sensitivity to training-stream order.} Ten
...
\subsection{5.4 Paired vs unpaired test}\label{paired-vs-unpaired-test}
...
Benchmark
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
Unpaired transition depth
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
Paired transition depth
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
Tightening
\end{minipage} \\
\midrule\noalign{}
...
\textbf{Table 5.} \emph{Paired vs unpaired transition depth at
...
Banking77/CLINC150; rule at 70.0\% vs ML at 75.6\% on ATIS) that even
the conservative unpaired test clears at the same checkpoint.

\begin{figure}
\centering
\pandocbounded{\includegraphics[width=\linewidth]{results/figure-4-paired-vs-unpaired.png}}
\caption{Figure 4. Paired McNemar (dark) vs unpaired-z (light) p-value
trajectories per benchmark on log-log axes; the dashed red horizontal is
alpha = 0.01. The paired curve is consistently below the unpaired curve
at every checkpoint, even when both ultimately clear alpha at the same
checkpoint resolution. HWU64 is the one benchmark where the empirical
first-clear gap is visible at the 250-checkpoint resolution (paired
clears at 250, unpaired at 500).}
\end{figure}
...
\subsection{5.5 Phase-1 LLM-shadow
capability}\label{phase-1-llm-shadow-capability}
...
§5.1 transition curve but answers the practitioner question ``what
should I shadow alongside the rule on day one?'' The second (§5.6) is
...
Phase 1 (\texttt{MODEL\_SHADOW}) lets a language model run alongside the
rule with zero risk: the model's prediction is logged but the rule still
...
\bottomrule\noalign{}
\endlastfoot
\texttt{llama3.2:1b} & 1B & 0.0\% & n/a \\
\texttt{gemma2:2b} & 2B & 42.0\% & n/a \\
\texttt{qwen2.5:7b} & 7B & \textbf{59.0\%} & \textbf{52.0\%} \\
\end{longtable}
}

\textbf{Table 6.} \emph{Zero-shot Phase-1 accuracy across model size and
...
\textbf{Regime II (ATIS, 26 labels, rule = 70\%).} No model in our
...
\textbf{Regime III (Banking77, 77 labels, rule = 1.3\%).} qwen2.5:7b's
...
outcome data via \sysname{}'s logging substrate while the trained ML head
...
(\texttt{OllamaAdapter}, \texttt{AnthropicAdapter},
\texttt{OpenAIAdapter}, \texttt{LlamafileAdapter}) is wired and ready;
extending the curve to Llama-70B / Mistral-Large / frontier APIs is one
method call away.

\subsection{5.6 Autoresearch on MLHead choice
(dogfood)}\label{autoresearch-on-mlhead-choice-dogfood}

The §9.5 \texttt{CandidateHarness} exists for the autoresearch use case:
...
classifier estimator): \texttt{TfidfLogReg} (the §4.3 incumbent),
\texttt{TfidfLinearSVC}, \texttt{TfidfMultinomialNB},
\texttt{TfidfGradientBoosting}. The harness ran each on the train split,
evaluated on the test split, and applied the same paired-McNemar gate at
\(\alpha = 0.01\) that gates the lifecycle's phase transitions. The
winner per benchmark is the accuracy-leading challenger that clears the
gate against the incumbent; otherwise the incumbent is held.
...
\bottomrule\noalign{}
\endlastfoot
AG News & 4 & \textbf{TfidfLogReg} (held) & (LinearSVC at +0.16\%, p =
0.44, did not clear) \\
TREC-6 & 6 & \textbf{TfidfLogReg} (held) & (LinearSVC at +2.6\%, p =
0.019, did not clear \(\alpha = 0.01\)) \\
Snips & 7 & \textbf{TfidfLogReg} (held) & (no challenger cleared) \\
codelangs & 12 & \textbf{TfidfLogReg} (held) & (LinearSVC at +0.72pp
didn't clear, \(p \approx 1.0\), only 1 discordant pair in its favor) \\
ATIS & 26 & \textbf{TfidfLinearSVC} & +5.4 pp \\
HWU64 & 64 & \textbf{TfidfLinearSVC} & +3.9 pp \\
Banking77 & 77 & \textbf{TfidfLinearSVC} & +1.8 pp \\
CLINC150 & 151 & \textbf{TfidfMultinomialNB} & +1.2 pp \\
\end{longtable}
}

\textbf{Table 7.} \emph{Autoresearch winners by benchmark.} Three
patterns emerge. First, mid-cardinality benchmarks (26 ≤ k ≤ 100)
consistently favor \texttt{TfidfLinearSVC}. Second, the
highest-cardinality benchmark favors \texttt{TfidfMultinomialNB} (the
Bayesian prior helps when each class has fewer training examples).
Third, low-cardinality benchmarks fail to clear the gate against the
incumbent: codelangs because the top three candidates (LogReg,
LinearSVC, GradientBoosting) cluster within \textasciitilde0.7 pp on a
small test set (139 rows) leaving few discordant pairs; Snips and AG
News because no challenger separates from the incumbent at
\(\alpha = 0.01\); TREC-6's LinearSVC was empirically +2.6 pp better but
at \(p = 0.019\) above \(\alpha\).

\begin{figure}
\centering
\pandocbounded{\includegraphics[width=\linewidth]{results/figure-6-autoresearch-winners.png}}
\caption{Figure 6. Autoresearch picks shape-dependent MLHead winners
across benchmarks. Cardinality on the x-axis (log), winning-candidate
accuracy on the y. Marker shape encodes which head won, with different
shapes for different cardinality regimes.}
\end{figure}

The finding is structural: \textbf{there is no universally best MLHead
...
configurable strategy: \texttt{postrule.MLHeadStrategy} is a Protocol,
\texttt{CardinalityMLHeadStrategy} is the shipped default with
thresholds informed by Table 7, and \texttt{head\_strategy=} plumbs into
\texttt{LearnedSwitch} so the strategy is consulted lazily on first
ML-head access. The strategy is itself a \sysname{} rule applied to
\sysname{}: a hand-coded threshold rule today, with a clear graduation
path (gate-driven head-selection learned over the autoresearch
trajectory) as the v1.x story.

\subsection{5.7 Beyond text: image-classification bench
(CIFAR-10)}\label{beyond-text-image-classification-bench-cifar-10}
...
\item
  \textbf{Rule:}
  \texttt{postrule.image\_rules.build\_color\_centroid\_rule} (companion
  to \texttt{postrule.benchmarks.rules.build\_reference\_rule} for
...
\item
  \textbf{ML head:} \texttt{postrule.ml.ImagePixelLogRegHead}. Flattens
  the 32×32×3 uint8 image to a 3072-dim float vector (normalized to
  {[}0, 1{]}), fits a multinomial logistic regression. sklearn-only, no
  torch dependency on the v1.0 install path.
\end{itemize}
...
\textbf{250} & 16.20\% & \textbf{24.60\%} & \textbf{7.0 × 10⁻⁴} \\
...
\textbf{Table 8.} \emph{CIFAR-10 transition curve.} The paired McNemar
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[width=\linewidth]{results/figure-7-cifar10-transition.png}}
\caption{Figure 7. CIFAR-10 transition curve. Panel (a): rule flat at
16.2\%, ML head rising from 19.4\% to 28.2\% as outcomes accumulate.
Vertical dashed line marks the first checkpoint at which the paired
McNemar gate fires at p \textless{} 0.01. Panel (b): paired McNemar
p-value trajectory on log scale, crossing alpha = 0.01 at 250 outcomes.}
\end{figure}
...
points (\texttt{register\_ml\_head}, \texttt{TfidfHeadBase} for text
variants, plain duck-typed objects for everything else). Audio and video
classification, and pretrained-embedding heads on images, are the
natural follow-ons; we defer them to a companion paper.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Category Taxonomy: Predicting Transition
Depth}\label{category-taxonomy-predicting-transition-depth}
...
\tightlist
\item
  \textbf{Label cardinality} (count of distinct labels).
\item
  \textbf{Rule keyword affinity} (does the label boundary admit stable
  ...
  keyword matches.
\item
  \textbf{Distribution stability} (KL divergence on a rolling window).
\item
  \textbf{Verdict latency} (seconds → days).
\item
  \textbf{Verdict quality} (direct human label \textgreater{} inferred
  outcome \textgreater{} heuristic proxy).
\item
  \textbf{Feature dimensionality} (low for hand-crafted features, high
  for embeddings).
\item
  \textbf{Input length} (utterance ≈ 5--15 words; news article ≈
...
labels), so a rule scoring \textasciitilde chance has \emph{weak}
affinity regardless of absolute percentage:
...
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Cardinality \textbackslash{} Keyword affinity
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\textbf{Strong} (\(\gg\)chance)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\textbf{Moderate} (≈ 2--5× chance)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\textbf{Weak} (≈ chance)
\end{minipage} \\
\midrule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Low} (\(k \le 12\)) & codelangs (87.8\% vs 8.3\% chance) &
TREC-6 (43.0\% vs 16.7\% chance) & Snips (14.3\% vs 14.3\% chance), AG
News (25.9\% vs 25.0\% chance) \\
\textbf{Mid} (\(12 < k \le 60\)) & ATIS (70.0\% vs 3.8\% chance) & --- &
--- \\
\textbf{High} (\(k > 60\)) & --- & --- & HWU64 (1.8\%), Banking77
(1.3\%), CLINC150 (0.5\%) \\
\end{longtable}
}

\textbf{Three regimes that fall out of this matrix:}

\begin{itemize}
\tightlist
\item
  \textbf{Regime A, Rule has fighting chance} (strong-or-moderate
  ...
  when keyword affinity is moderate (TREC-6, \(p \approx 10^{-16}\) at
  the first 250-outcome checkpoint) and slowly when keyword affinity is
  near-optimal (codelangs, where rule at 87.8\% and ML at
  \textasciitilde98\% leave few discordant pairs and the gate clears at
  400 outcomes).
\item
  \textbf{Regime A++, Rule near optimum} (strong keyword affinity, low
  cardinality). codelangs only in our suite. The rule is competitive
  with ML throughout. Graduation produces a \textasciitilde10pp lift;
  whether that lift justifies the engineering depends on per-decision
  cost. The lifecycle still helps (audit chain, structural fallback,
  drift-detection symmetry), but the \emph{transition curve} claim
  collapses to ``ML edges past the rule asymptotically'' rather than
  ``ML decisively replaces the rule.''
\item
  \textbf{Regime B, Rule at floor} (weak keyword affinity OR high
  ...
  \sysname{}'s role here is cold-start substrate, not graduation: outcome
...
\tightlist
\item
  \textbf{Direct human labels + strong/moderate keyword affinity} →
  Regime A; the McNemar gate clears within the first 1--2 checkpoints on
  every benchmark we tested.
\item
  \textbf{Weak keyword affinity OR high cardinality} → Regime B; rule is
  symbolic; transition depth is bounded below by \emph{outcome
  accumulation rate}, not statistical power.
\item
  \textbf{Strong keyword affinity AND modest cardinality} → Regime A++;
  consider whether ML graduation is worth the engineering effort given
  the small accuracy margin.
\item
  \textbf{High-stakes / regulated} → cap the lifecycle at Phase 4
  (\texttt{safety\_critical=True}); the rule remains the legal contract.
\end{itemize}
...
\section{Safety and Governance}\label{safety-and-governance}

\subsection{7.1 Safety-critical caps}\label{safety-critical-caps}
...
lifecycle caps at \(P_4\) (\texttt{ML\_WITH\_FALLBACK}). The reference
implementation refuses the \(P_4 \to P_5\) transition at construction
time when \texttt{safety\_critical=True} is set, and the \texttt{Gate}
protocol cannot be subverted by an operator without modifying source
code.

The construction-time refusal is a deliberate ergonomic choice: the
operator who would otherwise reach for \texttt{force\_advance(P\_5)} at
3 AM under incident pressure cannot. The contract is enforced by the
type system, not by discipline.

\subsection{7.2 Approval backends and audit
chain}\label{approval-backends-and-audit-chain}
...
transition takes effect. An \texttt{ApprovalBackend} protocol allows the
...
The audit chain is append-only by convention (POSIX \texttt{flock} +
atomic rotate; cryptographic tamper-evidence is left for v2). Every
...
\subsection{7.3 Circuit breaker}\label{circuit-breaker}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Eight benchmarks is not every category.} The taxonomy claim
...
\item
  \textbf{Most of the suite is text classification.} The eight-benchmark
  ...
  paper (§10.6).
\item
  \textbf{Rule construction is automated.} A thoughtful day-zero
  engineer might build a stronger keyword rule than
  \texttt{build\_reference\_rule} produces. The seed-size sensitivity
  analysis (§5.3) shows this does not change the regime conclusion, but
  it does shift the rule baseline modestly. We publish the rules for
  replication and scrutiny.
\item
  \textbf{The ML head is deliberately simple.} TF-IDF + logistic
...
\item
  \textbf{Verdict quality is treated as oracle.} In our experiments,
  ...
  \texttt{verifier=} slot supports an autonomous LLM-judge verdict
  source (and ships an evaluated default), but the noise model of
  inferred verdicts is a research direction in itself, complementary to
  this work.
\item
  \textbf{The McNemar gate's safety guarantee is per-step.} The
...
\item
  \textbf{One production case study; external customer deployments still
  pending.} §5.6's autoresearch on \sysname{}'s own MLHead choice is a
  real application of the framework: the \texttt{CandidateHarness}
  (§9.5) ran on actual data, with real paired-McNemar evidence,
  producing a verifiable promotion decision (\texttt{TfidfLinearSVC} for
  mid-cardinality, \texttt{TfidfMultinomialNB} for the
  highest-cardinality benchmark, \texttt{TfidfLogReg} held elsewhere)
  that the reference implementation now ships as the default
  \texttt{CardinalityMLHeadStrategy}. That is dogfooded production use
  within the project itself. What we do \emph{not} yet have is an
  external-customer case study: a third-party classification site with
  their own outcome plumbing and downstream signal. That is the natural
  follow-on.
\end{enumerate}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{The \sysname{} Reference
Implementation}\label{the-postrule-reference-implementation}

The reference implementation realizes the lifecycle as a Python library
(\texttt{pip\ install\ postrule}). Python is the v1.0 reference because
the empirical work in §5 is in Python, but the lifecycle, the Gate
protocol, and the audit-chain format are language-agnostic by design.
Client libraries for additional languages (TypeScript/JavaScript, Go,
Java, Rust) are on the roadmap so the gate primitive can be adopted at
the call site in any production stack, with a shared audit-chain format
that interoperates across them. The design decisions worth surfacing:

\subsection{\texorpdfstring{9.1 The \texttt{@ml\_switch}
decorator}{9.1 The @ml\_switch decorator}}\label{the-ml_switch-decorator}

How do you wire classification, dispatch, and the eventual rule-to-ML
migration through a single call site, so the same
\texttt{classify\_content(post)} invocation is what production code
calls on day one and on day thirty? The simplest invocation is a
decorator over the rule function:

\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{from}\NormalTok{ postrule }\ImportTok{import}\NormalTok{ ml\_switch}
\ImportTok{from}\NormalTok{ myapp.moderation }\ImportTok{import}\NormalTok{ publish, queue\_for\_review, block\_and\_notify}

\CommentTok{\# Content moderation: each label is paired with a downstream system}
\CommentTok{\# action whose miscost is immediate (target harm, censorship complaint,}
\CommentTok{\# or a moderator drowned in spillover).}
\AttributeTok{@ml\_switch}\NormalTok{(labels}\OperatorTok{=}\NormalTok{\{}
    \StringTok{"publish"}\NormalTok{: publish,}
    \StringTok{"review"}\NormalTok{:  queue\_for\_review,}
    \StringTok{"block"}\NormalTok{:   block\_and\_notify,}
\NormalTok{\})}
\KeywordTok{def}\NormalTok{ classify\_content(post: }\BuiltInTok{dict}\NormalTok{) }\OperatorTok{{-}\textgreater{}} \BuiltInTok{str}\NormalTok{:}
\NormalTok{    text }\OperatorTok{=}\NormalTok{ (post.get(}\StringTok{"body"}\NormalTok{) }\KeywordTok{or} \StringTok{""}\NormalTok{).lower()}
    \ControlFlowTok{if} \BuiltInTok{any}\NormalTok{(term }\KeywordTok{in}\NormalTok{ text }\ControlFlowTok{for}\NormalTok{ term }\KeywordTok{in}\NormalTok{ BANNED\_TERMS):}
        \ControlFlowTok{return} \StringTok{"block"}
    \ControlFlowTok{if}\NormalTok{ post.get(}\StringTok{"reporter\_count"}\NormalTok{, }\DecValTok{0}\NormalTok{) }\OperatorTok{\textgreater{}=} \DecValTok{3}\NormalTok{:}
        \ControlFlowTok{return} \StringTok{"review"}
    \ControlFlowTok{return} \StringTok{"publish"}

\NormalTok{classify\_content(post)  }\CommentTok{\# classifies AND fires the matching handler}
\end{Highlighting}
\end{Shaded}

Calling \texttt{classify\_content(post)} classifies the post and fires
the matching handler in a single call: classification and routing are
wired together at the decorator.
...
\texttt{classify\_content.switch.record\_verdict(record\_id,\ Verdict.CORRECT)}
...
The keyword rule
(\texttt{if\ any(term\ in\ text\ for\ term\ in\ BANNED\_TERMS):\ return\ "block"},
plus the three-report threshold for \texttt{"review"}) fails in
predictable, expensive ways. A slur with a deliberate typo or zero-width
character evades the banned-terms list and publishes; a coordinated
brigade of three reports flags a legitimate post into the review queue;
an emerging coded epithet sails through every keyword.
...
The body of \texttt{classify\_content} is the exact
\texttt{if}/\texttt{else} you would have inlined; the decorator is the
only addition. Everything else (outcome logging, gate evaluation,
lifecycle migration, audit chain, circuit breaker) happens for free.

\textbf{You write your rule once; \sysname{} upgrades it from a
hand-written keyword check to a trained ML head, in production, with no
rewrite.} On the §5 benchmarks the upgrade is a 70.0\% → 88.7\% lift on
ATIS, a 43.0\% → 85.2\% lift on TREC-6, and a sustained climb from
chance-floor to 81.9 to 87.7\% on the high-cardinality cluster
(Banking77, HWU64, CLINC150), measured against the as-shipped
HuggingFace splits at seed=100. The high-cardinality numbers are
sensitive to training-stream order (§5.4); the McNemar gate fires within
250 to 500 outcomes regardless of the rule's specific starting point.

Other dimensions ride along. \textbf{Latency} drops from LLM-class
...
\textbf{Per-call cost} drops from \(\$10^{-4}\) to \(\$10^{-2}\) per LLM
inference to essentially zero per ML-head call once the gate clears P5;
§10.3 lays out the at-scale extrapolation. \textbf{Drift detection}
ships in the same primitive (the gate fires in reverse when the rule
reclaims the lead). And \textbf{the call site itself is permanent}.
Production code calls \texttt{classify\_content(post)} on day one and on
day thirty just the same. The \emph{learned switch} on the inside (the
tuple \(S = (R, M, H, \phi)\) of §3.1) is what evolves.

\subsection{9.2 Storage and durability}\label{storage-and-durability}

The default storage is a bounded in-memory rotator (10,000 records
FIFO). Production deployments pass \texttt{persist=True} to switch to a
resilient file-backed store (\texttt{FileStorage}) wrapped in an
in-memory fallback (\texttt{ResilientStorage}) that buffers on disk
failure and drains on recovery. A \texttt{SqliteStorage} backend ships
for concurrent multi-process write workloads. The storage layer is
pluggable via the \texttt{Storage} protocol; users with existing audit
infrastructure (Kafka, Redshift, Snowflake) can plug those in.

\subsection{9.3 The verifier slot}\label{the-verifier-slot}
...
(\texttt{verifier=default\_verifier()}) that auto-detects a local Ollama
model (\texttt{qwen2.5:7b} by default; the verdict-quality frontier
across small-LLM judges is the subject of a companion write-up in
preparation) or an OpenAI/Anthropic API key, and uses the LLM-as-judge
pattern (Liu et al., 2023; Zheng et al., 2023) to produce verdicts for
every classification.

The same-LLM-as-classifier-and-judge bias is enforced at construction
time: the \texttt{JudgeSource} constructor refuses a judge model that
resolves to the same identity as the classifier, with a pointer to the
bias literature in the error message. Other verdict sources include
\texttt{WebhookVerdictSource}, \texttt{HumanReviewerSource} (for
cold-start labeling and periodic-drain workflows; refused on the inline
classify hot path because of its blocking semantics), and
\texttt{LLMCommitteeSource} for ensemble verdicts.

\subsection{9.4 MLHead strategy and factory
registry}\label{mlhead-strategy-and-factory-registry}
...
\tightlist
\item
  \textbf{\texttt{MLHeadStrategy} Protocol}:
  \texttt{select(records)\ -\textgreater{}\ MLHead}. Consulted lazily by
  \texttt{LearnedSwitch} on first ML-head access when the user passes
  \texttt{head\_strategy=...} instead of an explicit
  \texttt{ml\_head=...}.
\item
  \textbf{\texttt{CardinalityMLHeadStrategy}}: the shipped default.
  Picks \texttt{TfidfMultinomialNB} for high-cardinality + low-density
  (CLINC150-shape), \texttt{TfidfLinearSVC} for mid-cardinality (ATIS /
  HWU64 / Banking77), and falls through to \texttt{TfidfLogReg}
  otherwise. Thresholds are constructor kwargs so organizations with
  their own empirical findings can override.
\item
  \textbf{\texttt{FixedMLHeadStrategy(head)}}: bypasses the picker.
  Useful when an institution has run their own \texttt{CandidateHarness}
  and decided.
\item
  \textbf{Factory registry}: \texttt{register\_ml\_head(name,\ factory)}
  and \texttt{make\_ml\_head(name)}. Built-in heads are pre-registered
  (\texttt{tfidf\_logreg}, \texttt{tfidf\_linearsvc},
  \texttt{tfidf\_multinomial\_nb}, \texttt{tfidf\_gradient\_boosting},
  \texttt{image\_pixel\_logreg}); strategies can refer to heads by name
  without importing the class. Plugins extending \sysname{} register their
  own heads at import time.
\item
  \textbf{\texttt{TfidfHeadBase}}: public extension class. Subclass and
  override \texttt{\_build\_classifier()} to ship a TF-IDF +
  custom-estimator head; the shared pipeline (feature extraction,
  fit-from-correct-records, predict-with-confidence,
  \texttt{state\_bytes} / \texttt{load\_state} round-trip) is inherited.
  The four built-in TF-IDF heads use this base; user-contributed heads
  (\texttt{TfidfXGBoostHead}, \texttt{TfidfSGDClassifierHead}, etc.)
  plug in the same way.
\end{itemize}

The strategy is itself a \sysname{} rule applied to \sysname{}: a hand-coded
...
\subsection{\texorpdfstring{9.5 The
\texttt{CandidateHarness}}{9.5 The CandidateHarness}}\label{the-candidateharness}

For the autoresearch use case, \texttt{CandidateHarness} accepts a
stream of agent-proposed candidate classifiers, runs them in shadow mode
against live production traffic, and either promotes via the McNemar
gate or discards. This is the production substrate underneath the
autoresearch loops described by Karpathy (2025), Shinn et al.~(2023),
and Wang et al.~(2023).
...
\texttt{CandidateHarness} is built for:

\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{from}\NormalTok{ postrule }\ImportTok{import}\NormalTok{ ml\_switch, CandidateHarness}
\ImportTok{from}\NormalTok{ myapp.payments }\ImportTok{import}\NormalTok{ approve, decline, send\_to\_review}
\ImportTok{from}\NormalTok{ myapp.fraud\_research }\ImportTok{import}\NormalTok{ propose\_candidate  }\CommentTok{\# agent{-}driven proposer}

\AttributeTok{@ml\_switch}\NormalTok{(labels}\OperatorTok{=}\NormalTok{\{}
    \StringTok{"approve"}\NormalTok{: approve,}
    \StringTok{"decline"}\NormalTok{: decline,}
    \StringTok{"review"}\NormalTok{:  send\_to\_review,}
\NormalTok{\})}
\KeywordTok{def}\NormalTok{ screen\_payment(tx: }\BuiltInTok{dict}\NormalTok{) }\OperatorTok{{-}\textgreater{}} \BuiltInTok{str}\NormalTok{:}
    \ControlFlowTok{if}\NormalTok{ tx[}\StringTok{"amount"}\NormalTok{] }\OperatorTok{\textgreater{}} \DecValTok{10\_000} \KeywordTok{and}\NormalTok{ tx[}\StringTok{"country"}\NormalTok{] }\KeywordTok{not} \KeywordTok{in}\NormalTok{ ALLOWED:}
        \ControlFlowTok{return} \StringTok{"review"}
    \ControlFlowTok{if}\NormalTok{ tx[}\StringTok{"velocity\_24h"}\NormalTok{] }\OperatorTok{\textgreater{}} \DecValTok{10}\NormalTok{:}
        \ControlFlowTok{return} \StringTok{"decline"}
    \ControlFlowTok{return} \StringTok{"approve"}

\CommentTok{\# An agent proposes new screeners on a cadence. Each candidate is shadow{-}}
\CommentTok{\# evaluated against live traffic and promoted only if the paired{-}McNemar}
\CommentTok{\# gate clears at alpha=0.01. Failed candidates never touch user{-}visible}
\CommentTok{\# decisions; the incumbent screener stays in place.}
\NormalTok{harness }\OperatorTok{=}\NormalTok{ CandidateHarness(}
\NormalTok{    switch}\OperatorTok{=}\NormalTok{screen\_payment.switch,}
\NormalTok{    propose}\OperatorTok{=}\NormalTok{propose\_candidate,}
\NormalTok{    gate\_alpha}\OperatorTok{=}\FloatTok{0.01}\NormalTok{,}
\NormalTok{)}
\NormalTok{harness.run()  }\CommentTok{\# runs continuously in production}
\end{Highlighting}
\end{Shaded}
...
\subsection{9.6 Licensing and
governance}\label{licensing-and-governance}

The client SDK (everything end-users \texttt{import\ postrule} reaches)
is Apache 2.0. The analyzer / research / ROI components
(\texttt{analyzer.py}, \texttt{cli.py}, \texttt{research.py},
\texttt{roi.py}) are BSL 1.1 with a Change Date of 2030-05-01 (Change
License: Apache 2.0). The split is enforced at PR time by a CI workflow
that validates SPDX headers and a path allowlist.

The licensing rationale, threat model, and trademark policy are
published in \texttt{LICENSE.md}, \texttt{docs/THREAT\_MODEL.md}, and
\texttt{TRADEMARKS.md} respectively.

{[}patent application details omitted for anonymous review{]}. The
Apache 2.0 grant on the client SDK includes the patent grant standard to
that license; commercial use of the BSL-licensed analyzer / research /
ROI components is governed by the BSL terms until the 2030-05-01 Change
Date.
...
\subsection{10.1 What this paper changes for
practitioners}\label{what-this-paper-changes-for-practitioners}
...
automatically every 250 outcomes once \(n_{\min} = 200\) paired outcomes
...
\subsection{10.2 What this paper does not
change}\label{what-this-paper-does-not-change}
...
\subsection{10.3 Cost and latency at scale: graduation as economic
optimization}\label{cost-and-latency-at-scale-graduation-as-economic-optimization}
...
\sysname{} inherit \emph{both} graduations: the gate-validated accuracy
lift the paper measures, and a per-call cost-and-latency drop the paper
has not yet emphasized.

\textbf{Latency.} A model-classifier slot routes through a hosted LLM
...
implementation's \texttt{SklearnTextHead} shows p50 predict time under 2
ms (\texttt{tests/test\_latency.py}); switch overhead at
\texttt{ML\_WITH\_FALLBACK} and \texttt{ML\_PRIMARY} is on the order of
1--5 µs (\texttt{tests/test\_latency\_pinned.py}). The transition from a
Phase-2 LLM to a Phase-5 ML head is therefore a
\textbf{two-to-three-orders-of-magnitude latency reduction} at the call
site, for the same classification.

\textbf{Cost.} Per-call inference cost mirrors latency. A frontier-API
...
\textbf{A worked extrapolation, illustrative not predictive.} Take a
...
Pre-graduation operating cost is \(W \cdot c\) per day. At three
representative scales (\(c\) chosen as the midpoint of typical API
tiers, \(\$0.005\)/call):
...
Pre-graduation (LLM)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
Post-graduation (ML head)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
Annualized savings (illustrative)
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
\(10^8\) /day (large platform) & \$500,000/day & \(\approx 0\) &
\$182M/year \\
\end{longtable}
}
...
\(W \cdot c\), not a measurement; we present them to make the
...
\textbf{The same gate justifies both moves.} A practitioner who promotes
...
\subsection{10.4 Implications for the cascade-routing
literature}\label{implications-for-the-cascade-routing-literature}
...
introduced and retired}: a \texttt{MODEL\_PRIMARY} phase is structurally
identical to a FrugalGPT cascade with two stages (rule + LLM), and the
\(P_4 \to P_5\) transition is the moment when the cascade's escalation
...
\subsection{10.5 Implications for the autoresearch / agent
literature}\label{implications-for-the-autoresearch-agent-literature}
...
Trivedy's (2026) ``Better Harness'' framing (that harness hill-climbing
is the iterative discipline of evaluating the evaluator) operates one
level above ours; our gate is the underlying primitive. Lance Martin's
(2023) Auto-Evaluator is another primitive in this space, focused on
...
\subsection{10.6 Future work}\label{future-work}

\begin{itemize}
\tightlist
\item
  \textbf{External-customer production case studies.} §5.6 reports one
  production case study on \sysname{}'s own MLHead selection (the
  \texttt{CandidateHarness} running on the project's own data).
...
\item
  \textbf{Bidirectional drift handling.} The v1 reference implementation
  ...
  report.
\item
  \textbf{Multi-axis gating.} Production decisions involve more than one
  ...
  across axes (\(k\) axes, each bounded by \(\alpha\), give a per-cycle
  joint bound of \(k\alpha\)). A companion paper on the multi-axis
  safety story is the natural follow-on.
\item
  \textbf{Federated training.} Aggregating outcome pools across
  institutions without raw-data sharing. Does federation accelerate
  transitions?
\item
  \textbf{Adversarial transitions.} What happens to the gate under
  deliberate distribution shift?
\item
  \textbf{Verifier quality.} A companion write-up on small-LLM verifier
...
\item
  \textbf{Theoretical strengthening.} The safety theorem is
...
\item
  \textbf{Multi-modal extension and a companion paper.} The §5.7
...
\subsection{10.7 MLHead-strategy graduation as a recursive \sysname{}
problem}\label{mlhead-strategy-graduation-as-a-recursive-postrule-problem}

The §5.6 finding that the empirical winner depends on data shape
generalizes recursively. A \texttt{CardinalityMLHeadStrategy} is
structurally a \sysname{} rule applied to the MLHead-selection problem: a
hand-coded threshold table over data-profile features. Each time anyone
runs the autoresearch loop on a new dataset, they produce one record of
the form
\((\text{data-profile}, \text{winning-head}, \text{paired-McNemar evidence})\).
With a sufficient number of such records, the strategy itself becomes
graduable: train an \texttt{MLHeadSelectionHead} on the (profile →
winner) trajectory, gate its promotion against the cardinality rule via
the same paired McNemar test, and replace the rule with the model when
the gate clears.
...
surface (\texttt{MLHeadStrategy} is a Protocol;
\texttt{register\_ml\_head} is a registry; community contribution is a
question of process, not code), and (b) the evidence threshold for
clearing the gate against the cardinality rule is inversely proportional
to how shape-distinctive a data profile is, and we will not know that
until autoresearch findings accumulate beyond the eight benchmarks
reported here.
...
CLINC150) or by weak rule-keyword affinity (Snips, AG News). The
\sysname{} reference implementation, the transition-curve dataset, and the
...
for Deep Learning Using Calibrated Regression. \emph{ICML 2018}.
...
Liu, X., Eshghi, A., Swietojanski, P., \& Rieser, V. (2019).
Benchmarking Natural Language Understanding Services for Building
Conversational Agents. \emph{Proceedings of the 10th International
...
framework for large language models. {[}Recent MSR AI Frontiers Lab
work; final venue/citation in submission.{]}
...
\section{Reference Rules}\label{reference-rules}

The four reference rules are auto-generated by
\texttt{postrule.benchmarks.rules.build\_reference\_rule(seed=100)}. The
full source is at \texttt{src/postrule/benchmarks/rules.py}. We publish
the generated rules per benchmark in
\texttt{docs/papers/2026-when-should-a-rule-learn/rules/} for
replication.
...
\item
  ✓ Compute requirements: \textasciitilde30 minutes per benchmark on a
  2024-vintage laptop CPU.
\item
  ✓ All benchmark JSONLs and \texttt{paired\_mcnemar\_summary.json}
  released.
\item
  ✓ Benchmark harness reproduces the result:
  \texttt{postrule\ bench\ \{atis,banking77,clinc150,hwu64\}}.
\end{itemize}

\section{Code listings}\label{code-listings}

Key implementation references: -
\texttt{src/postrule/core.py::LearnedSwitch}: six-phase lifecycle. -
\texttt{src/postrule/gates.py::McNemarGate}: paired-McNemar gate. -
\texttt{src/postrule/research.py::run\_benchmark\_experiment}: the
harness used in §4--§5. -
\texttt{src/postrule/verdicts.py::JudgeSource}: LLM-as-judge with
same-model bias guardrail. -
\texttt{src/postrule/autoresearch.py::CandidateHarness}: autoresearch
substrate.
