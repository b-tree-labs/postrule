...
dataset, \texttt{\pkgname{}.benchmarks.rules.build\_reference\_rule}
...
\texttt{\pkgname{}.ml.SklearnTextHead}: TF-IDF features with sublinear
...
\texttt{\pkgname{}-transition-curves-2026.jsonl} accompanying this paper.
The benchmark harness ships in
\texttt{src/\pkgname{}/research.py::run\_benchmark\_experiment} and the
McNemar computation in \texttt{src/\pkgname{}/gates.py::McNemarGate}.
...
configurable strategy: \texttt{\pkgname{}.MLHeadStrategy} is a Protocol,
...
\item
  \textbf{Rule:}
  \texttt{\pkgname{}.image\_rules.build\_color\_centroid\_rule} (companion
  to \texttt{\pkgname{}.benchmarks.rules.build\_reference\_rule} for
...
\item
  \textbf{ML head:} \texttt{\pkgname{}.ml.ImagePixelLogRegHead}. Flattens
...
Implementation}\label{the-\pkgname{}-reference-implementation}

The reference implementation realizes the lifecycle as a Python library
(\texttt{pip\ install\ \pkgname{}}). Python is the v1.0 reference because
...
\begin{verbatim}
from \pkgname{} import ml_switch, CandidateHarness
...
The client SDK (everything end-users \texttt{import\ \pkgname{}} reaches)
...
problem}\label{mlhead-strategy-graduation-as-a-recursive-\pkgname{}-problem}
...
\texttt{\pkgname{}.benchmarks.rules.build\_reference\_rule(seed=100)}. The
full source is at \texttt{src/\pkgname{}/benchmarks/rules.py}. We publish
...
\item
  ...
  \texttt{\pkgname{}\ bench\ \{atis,banking77,clinc150,hwu64\}}.
\end{itemize}

\section{Code listings}\label{code-listings}

Key implementation references: -
\texttt{src/\pkgname{}/core.py::LearnedSwitch}: six-phase lifecycle. -
\texttt{src/\pkgname{}/gates.py::McNemarGate}: paired-McNemar gate. -
\texttt{src/\pkgname{}/research.py::run\_benchmark\_experiment}: the
harness used in §4--§5. -
\texttt{src/\pkgname{}/verdicts.py::JudgeSource}: LLM-as-judge with
same-model bias guardrail. -
\texttt{src/\pkgname{}/autoresearch.py::CandidateHarness}: autoresearch
substrate.
