...
**Postrule version:** 0.2.0
**Raw data:** `*.jsonl` in this directory
**Reproduce:** `postrule bench {banking77,clinc150,hwu64,atis}`
...
2. Streamed the full training set through Postrule, **recording every
...
- **"The rule never really worked"** (example: CLINC150 with 151
  ...
  without Postrule's outcome-logging in the first place*.

**Why this matters for Postrule as a product.** The two regimes map to
two very different sales conversations:
- To a team with a *medium-label, narrow-domain* classifier ("the rule
  works, we just wonder if we should replace it"): Postrule is the tool
  that gives a statistical answer to "yes, now."
- To a team with a *high-cardinality, broad-domain* classifier ("our
  rule is a joke but we shipped it because we had to"): Postrule is the
...
- **Training stream.** We feed training examples to Postrule one at a
...
- **Phase (0-5).** Postrule's six lifecycle stages: RULE → MODEL_SHADOW
...
- **Rule:** auto-generated by `postrule.benchmarks.rules.build_reference_rule`
  — seed of N training examples, top-5 distinctive keywords per label,
  fallback to the modal label in the seed window. Paper §4.2.
- **ML head:** `postrule.ml.SklearnTextHead` — TF-IDF + LogisticRegression.
...
1. **"The bigger the decision space, the more you need Postrule."**
   High-cardinality classification (70+ labels) has no meaningful
   day-zero rule. Postrule's primitive is the only abstraction that
   handles *start with nothing → graduate when ready* as one API surface.

2. **"For narrow-domain rules, Postrule tells you exactly when to graduate."**
   ATIS rules stay useful for a long time (70% is often "good enough"
   for triage). Postrule's statistical transition guards identify the
   crossover point empirically rather than by intuition.

3. **"Postrule is the only production primitive that maps to this progression."**
...
Where Postrule pays off fastest:
...
- **Medium-cardinality classification** (10–50 labels) with at least
  ...
  day-one; Postrule shows you the crossover point.
- **High-cardinality classification** (50+ labels) where you need a
  ...
  rule is a fig leaf, but Postrule's outcome-logging is what lets you
  build the ML classifier that replaces it.
- **Safety-critical decision points** where the rule is a non-negotiable
  floor. Postrule's `safety_critical=True` caps graduation at
  ML_WITH_FALLBACK, so the rule remains the contract.

Where Postrule is less differentiated:
...
GPT-4-class) is the natural follow-up. The Postrule `OllamaAdapter`,
...
is defined in ``src/postrule/gates.py::McNemarGate``.
...
| Org size | Typical classification sites | Postrule-fit sites |
|---|---|---|
...
**Tier 1 — slam-dunk fit for Postrule's graduated-autonomy primitive:**
...
monitoring + circuit breaker). With Postrule, the first site costs 1–2
...
the internal scan) shipped with Postrule wrapping the rule in Phase 0
