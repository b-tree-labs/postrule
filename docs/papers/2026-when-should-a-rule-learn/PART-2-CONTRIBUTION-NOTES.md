...
- **Fresh empirical evidence (routing slice, banking77; weak=haiku, strong=sonnet, n=300):**
  ...
  - **But cheap signals can't capture it (two probes, both honest negatives):**
    - *Passive features (P0):* TF-IDF **and** sentence-embedding routers both land *below* always-weak — dominated by the trivial baseline. The policy is **not identifiable from the query text**.
    - *Uncertainty cascade (P0-B, MMLU, n=120):* escalating the 12% least self-consistent queries lifts accuracy only ~1 pt (0.57→0.58) vs an 18-pt oracle headroom — the cheap model is often *confidently wrong*, so self-consistency is a **weak** deferral signal.
    - → **Identifiability of the routing policy requires a *calibrated* signal (true logprobs) or *learned off-policy* deferral — cheap shortcuts fail.** That's the sharper claim: not just "text fails" but "the *easy* observables fail; restoring identifiability takes real work." Cite FrugalGPT, learning-to-defer (Madras; Mozannar & Sontag) — we claim no new deferral method, just the framing + the measured negatives.
...
- **Methodological-honesty tidbits:** small-n pilots misled us **three** times — audio LLM 0.60→**0.47** (full fold), routing router 85%→**chance** (n=300), and the cascade "25%→full-strong" → **+1pt** (n=8→120). Report the adequately-powered number, not the lucky pilot — same discipline as "test the *organic* gate." (Worth a methods sentence: a recurring theme of *this* work is that small samples over-state graduation/routing wins; the honest evals reverse them.)
