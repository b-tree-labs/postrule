# Initial Analysis — what `postrule analyze` found in this codebase
...
>
> Run `postrule init src/triage.py:triage_rule` to wrap your first site.

## Top opportunities (ranked by Postrule's opportunity model)
...
|---|---|---|---|---|---:|---|---:|---|
| 1 | `src/triage.py:43 triage_rule` | P1 | narrow (4) | hot | **5.00** | ~14 days | **$4,200** | `postrule init src/triage.py:triage_rule` |
| 2 | `src/router.py:88 ticket_router` | P3 | narrow (5) | hot | **5.00** | ~16 days | **$1,800** | `postrule init src/router.py:ticket_router` |
| 3 | `src/safety.py:56 output_safety_rule` | P1 | narrow (3) | warm | **3.50** | ~18 days | **$850** | `postrule init src/safety.py:output_safety_rule` |
| 4 | `src/notifications.py:12 priority_picker` | P1 | narrow (3) | warm | **3.50** | ~21 days | **$520** | `postrule init src/notifications.py:priority_picker` |
| 5 | `src/uploads.py:33 file_type_classifier` | P3 | narrow (4) | warm | **3.50** | ~25 days | **$340** | `postrule init src/uploads.py:file_type_classifier` |
| 6 | `src/intents.py:15 intent_classifier` | P1 | medium (12) | warm | 2.80 | ~32 days | **$2,400** | `postrule init src/intents.py:intent_classifier` |
| 7 | `src/incidents.py:120 severity_classifier` | P4 | narrow (3) | warm | 2.80 | ~28 days | **$1,900** | `postrule init src/incidents.py:severity_classifier` |
| 8 | `src/feedback.py:55 feedback_categorizer` | P4 | narrow (4) | warm | 2.80 | ~30 days | **$680** | `postrule init src/feedback.py:feedback_categorizer` |
...
adapter). Switch with `postrule report --pro-forma --model claude-haiku-4.5`
...
This section grows as you `postrule init` sites. The cohort-tuned defaults
...
After `postrule init` on the first site, this file is replaced by
`postrule/results/_summary.md` (the "cockpit" view). Each wrapped switch
gets its own report card at `postrule/results/<switch>.md` that fills in
over time:
...
*Regenerate with `postrule analyze --report`. This file is a snapshot of
what was found at scan time; it doesn't update as your code changes.
For ongoing status, use `postrule report --summary`. Each scan writes
