...
4. ``format_report`` renders the human-readable text expected by the
   ``postrule report`` CLI verb.

The benchmark module collides with the existing ``postrule.benchmarks``
package (which holds the public-dataset loaders), so the harness lives
at ``postrule.benchmarks.harness`` and is re-exported from the package
init for both ``from postrule.benchmarks import generate_benchmark_module``
and ``from postrule.benchmarks.harness import ...``.
"""
...
class TestGenerateBenchmarkModule:
    """The codegen path called from ``postrule init --with-benchmarks``."""
    ...
    def test_includes_imports_for_switch_and_pytest(self, tmp_path: Path) -> None:
        ...
        out_path = tmp_path / "__postrule_generated__" / "x__y_bench.py"
        generate_benchmark_module(
            out_path=out_path,
            source_module="x",
            source_function="y",
            switch_class_name="YSwitch",
            switch_module="x___postrule_generated__.x__y",
...
class TestRunBenchmark:
    ...
    def test_produces_jsonl_line_with_expected_fields(
        ...
        from postrule.benchmarks import run_benchmark
        ...
        bench_dir = runtime_root / "postrule" / "synth" / "benchmarks"
...
def _write_bench_line(
    ...
    *,
    ...
    bench_dir = runtime_root / "postrule" / switch / "benchmarks"
...
class TestAggregateReport:
    ...
    def test_identifies_graduation_event_and_cost_delta(self, tmp_path: Path) -> None:
        from postrule.benchmarks import aggregate_report
...
class TestRealCostMeasurement:
    ...
    def test_real_cost_path_records_sum_from_stub_adapter(
        ...
        (pkg_dir / "__init__.py").write_text("")
        (pkg_dir / "synth_switch.py").write_text(
            "from postrule import Switch, Phase\n"
            "from postrule.models import ModelPrediction\n"
            "\n"
            "class StubModel:\n"
...
class TestInitWithBenchmarksFlag:
    """``postrule init --auto-lift --with-benchmarks`` writes the bench
